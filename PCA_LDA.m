...
% link for subplots: https://de.mathworks.com/help/matlab/creating_plots/combine-multiple-plots.html
figure;   
ax1 = subplot(2,2,1);
...
figure;
scatter(feature_mat(1:190,1), feature_mat(1:190,2), 'b');  % no error
hold on;
scatter(feature_mat(191:300,1), feature_mat(191:300,2), 'r'); % error
...
[~,n] = min(M);  % n is the column containing the min value of the matrix
...
%feature_mat_recall = (feature_mat_recall - min(min(feature_mat_recall)))/(max(max(feature_mat_recall))- min(min(feature_mat_recall)));
...
scatter(feature_mat_recall(Results_no_error_indx,1), feature_mat_recall(Results_no_error_indx,2), 'b');
hold on;
scatter(feature_mat_recall(Results_error_indx,1), feature_mat_recall(Results_error_indx,2), 'r');
legend('No error,-1','error,1')
xlabel('1st best feature') % x-axis label
ylabel('2nd best feature') % y-axis label
