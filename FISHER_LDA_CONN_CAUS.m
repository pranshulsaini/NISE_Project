...
for i = 1:n_no_error % there will be a connectivity matrix for every no error trial
    for j = 1: size(data_Error,1)    % chan1
        for k =  1: size(data_Error,1)  % chan2 
            [caus_mat(i,j,k),P(i,j,k)] = granger_cause(squeeze(data_noError(j,:,i)),squeeze(data_noError(k,:,i)), 0.05,5); % transpose make channels as columns
        end
    end
end

a = squeeze(caus_mat(100,:,:));

for i = 1:n_error % there will be a causality matrix for every error trial
     for j = 1: size(data_Error,1)    % chan1
        for k =  1: size(data_Error,1)  % chan2 
            [caus_mat(i+n_no_error,:,:),P(i+n_no_error,:,:)] = granger_cause(squeeze(data_Error(j,:,i)),squeeze(data_Error(k,:,i)), 0.05,5); % 
        end
     end
end
...
[max, locs] = maxNvalues(caus_fish_val,n_caus_feat);
...
for i = 1: n_caus_feat
    caus_mat_final(:,i) = caus_mat(:,locs(i,1),locs(i,2));
end

caus_mat_final_mean = squeeze(mean(caus_mat_final))';  % needed for topoplot
...
%% Testing the performance of features

% training based on only connectivity features
model = trainShrinkLDA(caus_mat_final,labels,lambda);  % final model
[y] = predictShrinkLDA(model,caus_mat_final);
performance_error = sum(y' ~= labels)/300;  % This is the misclassification rate
